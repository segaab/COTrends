...
# Asset definitions (COT market names mapped to Yahoo common ticker for prices)
assets = {
...
def fetch_yahooquery_data(ticker, start_date, end_date, max_attempts=3):
    ...
    while attempt < max_attempts:
        try:
            t = Ticker(ticker)
            # Fetch historical data with interval 1d and adjusted close
            hist = t.history(start=start_date, end=end_date, interval='1d')
            if hist.empty:
                logger.warning(f"No data returned for {ticker} from yahooquery.")
                return pd.DataFrame()
            # If multiple tickers returned, select ticker
            if isinstance(hist.index, pd.MultiIndex):
                hist = hist.loc[ticker]
            hist = hist.reset_index()
            hist.rename(columns={"date": "date"}, inplace=True)
            # Sometimes openInterest might not be available; we'll rely on COT for that
