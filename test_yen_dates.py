...
# Asset definitions (COT market names mapped to Yahoo futures contract tickers)
assets = {
    ...
    # Crypto Futures (using spot proxies)
...
def fetch_cot_data(market_name, max_attempts=3):
    ...
    while attempt < max_attempts:
        try:
            results = client.get("6dca-aqww", where=where_clause, order="report_date_as_yyyy_mm_dd DESC", limit=1000)
            if results:
...
def merge_cot_price(cot_df, price_df):
    ...
    cot_merge = cot_merge.sort_values('date')  # Ensure sorting for merge_asof
...
def calculate_health_gauge(df):
    ...
    return health_score * 10  # scale 0-10
...
def main():
    ...
    st.markdown(f"**Data Range for Calculations:** {start_date} to {end_date}")
    ...
    for cot_name, ticker in assets.items():
        ...
        adj_end = min(end_date, cot_end + datetime.timedelta(days=7))  # to cover slight lag in reporting
        ...
        st.metric("Latest Health Gauge Score (0-10)", f"{latest_health:.2f}")
